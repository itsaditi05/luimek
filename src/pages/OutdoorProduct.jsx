...
const ProductCard = ({ product }) => {
  ...
  const mrp = Math.round(numericPrice / 0.7).toLocaleString(); 
  ...
  return (
    // 👇 LINK UPDATED TO /outdoor
    <Link to={`/outdoor/${product.id}`} className="block h-full">
      <div 
...
const OutdoorProduct = () => {
  ...
  useEffect(() => {
    document.title = "Outdoor Lighting Collection | Luimek";
  }, []);

  const filteredProducts = useMemo(() => {
    let data = [...outdoorProducts];
  ...
  return (
    <div className="bg-black min-h-screen text-white pt-24 pb-20">
      ...
      <div className="container mx-auto px-4 flex gap-8">
            ...
            <div className="space-y-2">
              {[
                ...
                <label key={opt.val} className="flex items-center gap-3 cursor-pointer group">
                  ...
                  <input type="radio" name="price" value={opt.val} checked={priceFilter === opt.val} onChange={() => setPriceFilter(opt.val)} className="hidden" />
                  <span className={`text-sm ${priceFilter === opt.val ? 'text-white' : 'text-gray-400 group-hover:text-white'}`}>{opt.label}</span>
                </label>
              ))}
            </div>
          </div>
        ...
        <div className="flex-1">
          <div className="hidden lg:flex justify-between items-end mb-6">
            <div>
              <h1 className="text-3xl font-bold text-yellow-400">Outdoor Collection</h1>
              <p className="text-gray-400 mt-1">Durable & stylish lighting for your exteriors</p>
            </div>
          </div>
          ...
          {filteredProducts.length > 0 ? (
            ...
            <div className="text-center py-20">
              ...
              <button onClick={() => setPriceFilter("all")} className="mt-4 text-yellow-500 underline">Clear Filters</button>
            </div>
          )}
        </div>
      </div>
      ...
      <AnimatePresence>
        {isMobileFilterOpen && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/80 z-50 lg:hidden" onClick={() => setMobileFilterOpen(false)} />
            <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 500 }} className="fixed bottom-0 left-0 w-full bg-[#151515] z-50 rounded-t-2xl p-6 lg:hidden border-t border-gray-700">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-bold">Filters</h2>
              ...
              <div className="space-y-3 mb-8">
                {[
                  ...
                  <label key={opt.val} className="flex items-center gap-3 p-3 bg-black rounded-lg border border-gray-800">
                    <input type="radio" name="mobilePrice" checked={priceFilter === opt.val} onChange={() => setPriceFilter(opt.val)} className="accent-yellow-500 w-5 h-5" />
                    <span className="text-white">{opt.label}</span>
                  </label>
                ))}
              </div>
              <button onClick={() => setMobileFilterOpen(false)} className="w-full bg-yellow-500 text-black font-bold py-3 rounded-lg">Apply Filters</button>
            </motion.div>
          </>
        )}
      </AnimatePresence>
